...
while True:
    ...
    if any(pX <= X <= (pX + width) for X in range (foodX, foodX + Swidth)) and any(pY <= y <= (pX + height) for y in range (foodY, foodY + Swidth)) and any(pY <= y <= (pY + height) for y in range (foodY, foodY + Sheight)):
        pygame.quit()
        sys.exit()
